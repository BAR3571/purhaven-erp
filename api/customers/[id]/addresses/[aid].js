...
export default async function handler(req, res) {
  ...
  if (!Number.isFinite(customerId) || !Number.isFinite(addressId)) {
  ...
  if (req.method === 'PUT') {
    ...
    const rows = await sql`
      UPDATE erp_customer_addresses SET
        label = ${b.label || null},
        type = ${type},
        line1 = ${b.line1 || null},
        line2 = ${b.line2 || null},
  ...
  if (req.method === 'DELETE') {
    const rows = await sql`
      DELETE FROM erp_customer_addresses
      WHERE id = ${addressId} AND customer_id = ${customerId}
      RETURNING id
    `;
    if (rows.length === 0) return res.status(404).json({ error: 'Address not found' });
