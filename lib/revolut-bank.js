  ...
  for (const t of txns) {
    ...
    for (const leg of (t.legs || [])) {
      ...
      if (amountPence > 0) {
        ...
        // that isn't already marked paid (avoids double-linking manual payments).
        ...
        `;
        if (soRows[0]) {
          matchedSo = soRows[0].id;
          // Also stamp the SO so both sides show the payment
          await sql`
            UPDATE erp_sales_orders
            SET paid_at = ${txnDate}::timestamptz,
                paid_amount_pence = ${amountPence},
                payment_method = 'Revolut (bank feed)',
                payment_ref = ${t.reference || legId},
                updated_at = NOW()
            WHERE id = ${matchedSo}
          `;
        }
