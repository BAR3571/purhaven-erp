...
export async function getProductWithRelations(id) {
  ...
  const serials = await sql`
    SELECT s.*,
           ...
           ps.serial_number AS parent_serial_number
    FROM erp_product_serials s
    LEFT JOIN erp_customers c ON c.id = s.despatched_to_customer_id
    LEFT JOIN erp_product_serials ps ON ps.id = s.parent_serial_id
    WHERE s.product_id = ${id}
    ORDER BY
      CASE s.status
        WHEN 'in_stock' THEN 1
        WHEN 'despatched' THEN 2
  ...
  return { ...product, stock, movements, children, serials };
}
